...
class CollectorServicer(gnmi_pb2_grpc.gNMIServicer):
    ...
    def stream(self, stub):
        ...
        for response in stub.Subscribe(request_iterator):
            ...
            if response.update:
                yield filterAndPackage(response.update) 
            else:
                pass
    ...
    def Subscribe(self, request_iterator, context):
        ...
        pool = ThreadPool(2)
        stubs = [stub1, stub2]
        global PAIR_LIST #the list is not explicitly put in shard memory w/ multiprocessing.Array()? does just global work.
        PAIR_LIST = pool.map(self.stream, stubs)
